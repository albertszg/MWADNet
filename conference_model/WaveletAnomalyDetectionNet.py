...
from utils.utils_ae import metrics_calculate, lr_scheduler_choose
...
from conference_model.Script_MbandWN_frequencyanomaly_validation import coefficient_visul_batch as coefficient_visul
from conference_model.Script_MbandWN_frequencyanomaly_validation import plot_box,fft_fignal_plot,plot_energydistribution
import math
import pandas as pd
'''
WADNet(kernelInit=None, m=[3], k=[2],level=None, mode='DWT',kernTrainable=True,  kernelsConstraint='CF',
...
class Mband_AE(object):
    def __init__(self, data_loader, savedir, args):
        ...
        ae = WADNet(kernelInit=args.KernelInit, m=args.m, k=args.k,level=args.level, mode=args.mband_mode,kernTrainable=args.kernTrainable, kernelsConstraint=args.kernelsConstraint,
                 coefffilter=args.coeff_filter,device=self.device,realconv=args.realconv,t=args.threshold_t,initHT=args.ThreInit,initHT1=args.ThreInit1, trainHT=args.trainHT,threshold=args.threshold,seqlen=args.window_size)
    ...
    def ae_train(self, x):
        # 主程序训练生成器：对抗loss加上根据重构误差自适应的加权MSE的loss
        self.ae_optimizer.zero_grad()
        # sigPred, coeff, lossFilter
        re_x, z, lossFilters, hard_threshold_max,coefficient_for_visualization = self.ae(x)  # 重构变量和中间变量

        traning_mse = self.mse(re_x, x)
        traning_sparse = self.coff_sparsity(z)
        traning_filter = lossFilters

        loss = traning_mse + self.sparsitylosscoeff * traning_sparse - self.hardcoeff * hard_threshold_max + self.filterlosscoeff*lossFilters
        loss.backward()
    ...
    def validate(self):
        ...
        for batch_idx, (x, _) in enumerate(self.data_loader['val']):
            ...
            re_values = self.value_reconstruction_val(x) #获得重构值 re_values
            self.val_loss += self.mse(x, re_values).item() * x.size(0)
            num_batch += x.size(0)
        self.val_loss = self.val_loss / num_batch
        self.ae.train()  # 训练模式

    def test(self, load_from_file=False, last_epoch=False,titleadd=''):
        if load_from_file:
    ...
    def load_best_model(self):
        ...
        ae = WADNet(kernelInit=kernel_init, m=self.args.m, k=self.args.k, level=self.args.level, mode=self.args.mband_mode,
             ...
             initHT=self.args.ThreInit, initHT1=self.args.ThreInit1,trainHT=self.args.trainHT, threshold=self.args.threshold)
        ae.load_state_dict(
    ...
    def plot_reconstruction_trainset(self, figure_number=1, channel=0, test=False):
        '''
        可视化 训练集 重构效果图 只有正常
        figure_number:可视化的图片数量
        channel:样本的通道
        '''
        self.ae.eval()
    ...
    def show_threshold(self):
        logging.info('*'*6+'showing threshold'+'*'*6)
        threshold_number = self.ae.thre.l.size(0)
        print('threshold after optimizaiton')
        print('左侧内侧')
        print(self.ae.thre.l.view(threshold_number))
        print('左侧带通范围')
        print(self.ae.thre.l1.view(threshold_number))
        print('右侧内侧')
        print(self.ae.thre.r.view(threshold_number))
        print('右侧带通范围')
        print(self.ae.thre.r1.view(threshold_number))
        ...
        if batch_number>1:
            for batch_idex, (x, y) in enumerate(self.data_loader['test']):
                if batch_idex == 0:
                    ...
                    reconstruct_sample, coeff,_,_,threded_coeff = self.ae(torch.unsqueeze(x.to(self.device),dim=3))
                    reconstruct_sample = reconstruct_sample.detach().cpu().numpy()
                elif batch_idex == batch_number - 1:
                    sample_last = x.detach().numpy()
                    last_batch_label = y
                    reconstruct_sample_last, coeff,_,_,threded_coeff = self.ae(torch.unsqueeze(x.to(self.device),dim=3))
            ...
            for i in range(figure_number):
                ...
                coefficient_visul(coeff, mode='WPT', m=self.args.m, title='ori_'+title, batch_idx=i)
                coefficient_visul(threded_coeff, mode='WPT', m=self.args.m,title='thresholed_'+title,batch_idx=i)  # ,prefined_sort=[1,2,0,3]
                plot_box(coeff,batchidx=i,title=title, m=self.args.m)
                plot_energydistribution(coeff, batchidx=i,title='ori_'+title, m=self.args.m)
                plot_energydistribution(threded_coeff, batchidx=i,title='thresholed_'+title, m=self.args.m)
            ...
            for i in range(figure_number):
                ...
                coefficient_visul(coeff, mode='WPT', m=self.args.m, title='ori_' + title,
                                  batch_idx=i)  # ,prefined_sort=[1,2,0,3]
                coefficient_visul(threded_coeff, mode='WPT', m=self.args.m, title='thre_'+title,batch_idx=i)  # ,prefined_sort=[1,2,0,3]
                plot_box(coeff, batchidx=i,title=title, m=self.args.m)
                plot_energydistribution(coeff, batchidx=i,title='ori_' +title, m=self.args.m)
                plot_energydistribution(threded_coeff, batchidx=i,title='thre_'+title, m=self.args.m)
        else:
            #只有一个batch时候
            for batch_idex, (x, y) in enumerate(self.data_loader['test']):
                first_batch_label = y
            ...
            for i in range(figure_number):  # 头部的重构效果
                ...
                coefficient_visul(coeff, mode='WPT', m=self.args.m, title='ori_'+title,batch_idx=i)  # ,prefined_sort=[1,2,0,3]
                coefficient_visul(threded_coeff, mode='WPT', m=self.args.m, title='thre_'+title,batch_idx=i)  # ,prefined_sort=[1,2,0,3]
                plot_box(coeff, batchidx=i,title=title, m=self.args.m)
                plot_energydistribution(coeff, batchidx=i,title='ori_'+title, m=self.args.m)
                plot_energydistribution(threded_coeff, batchidx=i,title='thre_'+title, m=self.args.m)
            ...
            for i in range(figure_number):  # 尾部的重构效果
                ...
                coefficient_visul(threded_coeff, mode='WPT', m=self.args.m, title='ori' + title,
                                  batch_idx=i)  # ,prefined_sort=[1,2,0,3]
                coefficient_visul(coeff, mode='WPT', m=self.args.m, title='thre'+title,
                                  batch_idx=i)  # ,prefined_sort=[1,2,0,3]
                plot_box(coeff, batchidx=i,title=title, m=self.args.m)
                plot_energydistribution(threded_coeff, batchidx=i, title='ori' +title, m=self.args.m)
                plot_energydistribution(coeff, batchidx=i,title='thre'+title, m=self.args.m)