...
class Mband(nn.Module):
    """
        ...
        给分解器组
    """
    def __init__(self, De=None, Re=None, M=4,k=2, realconv=False, trainable=False,device = 'cuda',seqlen=1000):
        super(Mband, self).__init__()
        self.realconv=realconv
        self.M=len(De)
        self.regularity=k
        self.trainable=trainable
        self.signal_size=seqlen
        ...
        for i in range(self.M):
            ...
            DeFilter.append(Analysis(kernel=De[i], M=self.M, realconv=self.realconv, device=device,z_dim=seqlen))
    ...
    def forward(self, input, decomposition=1):
        ...
        if decomposition==1:
            ...
            x_re = self.ReFilter[0](input[0],self.signal_size) #一层mband单元同时包含分解和合成，对偶的存在
...
class Analysis(nn.Module):
    ...
    def __init__(self, kernel, M, realconv=False, device ='cuda',z_dim=80, **kwargs):
        ...
        self.dim = z_dim
        self.fc1 = nn.Linear(z_dim, int(z_dim/16))
        self.fc2 = nn.Linear(int(z_dim/16), int(z_dim/16))
        self.fc3 = nn.Linear(int(z_dim/16), z_dim)

    def forward(self, input):
        temp1 = self.ConvLayer(input, self.realconv)
        # assert self.dim == temp1.size(2)
        # temp = self.fc1(torch.squeeze(temp1, 3))
        # temp = self.fc2(temp)
        # tmep = self.fc3(temp)
        # return torch.unsqueeze(tmep,3)+temp1
        return temp1
...
class WaveletThreshold(nn.Module):
    ...
    m：band-1，只进行高频去噪
    """
    def __init__(self, init=None, trainBias=True, t=100.0, m=1, threshold=True,seqlen=10,**kwargs):
        super(WaveletThreshold, self).__init__()
        if isinstance(init,float) or isinstance(init,int):
            self.left = Parameter(torch.ones(m, 1, 1, seqlen, 1)*init,requires_grad=trainBias)
            self.right = Parameter(torch.ones(m, 1, 1, seqlen, 1)*init,requires_grad=trainBias)
        else:
            self.left  = Parameter(torch.ones(m, 1, 1, seqlen, 1), requires_grad=trainBias)
            self.right = Parameter(torch.ones(m, 1, 1, seqlen, 1), requires_grad=trainBias)
