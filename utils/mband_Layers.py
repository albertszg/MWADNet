...
class WaveletThreshold_plus_patent(nn.Module):
    """
    with normalization
    Learnable Hard-thresholding layers = biased Relu
    :param
    ...
    def __init__(self, init=None,init1=None, trainBias=True, t=10.0, m=1, threshold=True,norm=False,**kwargs):
        ...
        if isinstance(init,float) or isinstance(init,int):
            ...
            self.r = 0.0
            self.r1 = 0.0
        self.fc_l = nn.Sequential(nn.Linear(1, 8),nn.ReLU(),nn.Dropout(0.3),nn.Linear(8, 8),nn.ReLU(),nn.Dropout(0.3),nn.Linear(8, m))
        self.fc_l1 = nn.Sequential(nn.Linear(1, 8),nn.ReLU(),nn.Dropout(0.3),nn.Linear(8, 8),nn.ReLU(),nn.Dropout(0.3),nn.Linear(8, m))
        self.fc_r = nn.Sequential(nn.Linear(1, 8),nn.ReLU(),nn.Dropout(0.3),nn.Linear(8, 8),nn.ReLU(),nn.Dropout(0.3),nn.Linear(8, m))
        self.fc_r1 = nn.Sequential(nn.Linear(1, 8),nn.ReLU(),nn.Dropout(0.3),nn.Linear(8, 8),nn.ReLU(),nn.Dropout(0.3),nn.Linear(8, m))
        self.trainBias = trainBias # 是否训练阈值
        self.t=torch.tensor(t) #温度t
        self.threshold=threshold #是否使用阈值操作
        self.m=m # Mband的带数
    ...
    def forward(self, input, condition, NodefineT=True,HT=1.0,HT1=100.0):
        '''
        :param input: 输入x
        :param condition:输入工况condition
        :param NodefineT: 是否有定义的阈值t，若有则使用指定的阈值，否则适应可学习的阈值
        :param HT:指定的硬阈值
        :return:阈值处理后的输入
        '''
        ...
        # return torch.multiply(input,torch.sigmoid(10*(input-self.thrP))+torch.sigmoid(-10*(input+self.thrN)))
        assert len(input)==self.m
        self.l = torch.abs(self.fc_l(condition)).view(self.m,-1,-1,-1,-1)
        self.l1 = torch.abs(self.fc_l1(condition)).view(self.m,-1,-1,-1,-1)
        self.r = torch.abs(self.fc_r(condition)).view(self.m,-1,-1,-1,-1)
        self.r1 = torch.abs(self.fc_r1(condition)).view(self.m,-1,-1,-1,-1)
        if NodefineT:
            if self.threshold:
                # return [wavelet_threshold_normal_min_max(input[m_in_level], self.t, left=self.l[m_in_level], right=self.r[m_in_level],
                #                                          left1=self.l1[m_in_level], right1=self.r1[m_in_level], norm=self.norm) for m_in_level in range(len(input))]
                return [wavelet_threshold_normal_min_max(input[m_in_level], self.t, left=self.l[m_in_level],
                                                     right=self.r[m_in_level],
                                                     left1=self.l[m_in_level]+self.l1[m_in_level], right1=self.r[m_in_level]+self.r1[m_in_level],
                                                     norm=self.norm) for m_in_level in range(len(input))]
